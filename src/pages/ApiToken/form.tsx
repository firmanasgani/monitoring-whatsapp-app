...
const ApiTokenForm = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        ...
        if(token) {
            ...
            axios.post(`${process.env.REACT_APP_API_URL}/api/openapi`, data, {
                headers: {
  ...
  return (
    <LayoutPage>
      ...
      <div >
        <form onSubmit={handleSubmit} className="p-6 bg-white rounded-lg shadow-md mb-4 flex flex-col gap-4 flex-start w-full"> 
          ...
          {error && (<p className="text-red-600">Tanggal tidak bisa sama atau lewat dari hari ini!</p>)}
        </form>
      </div>
      ...
      {postData ? (
        <div className="p-6 bg-white rounded-lg shadow-md mb-4 flex flex-col justify-between">
          ...
          <div className="mb-4">
            <p>Token key: <b>{dataAfterPost.api_key}</b></p>
            <p>Secret Key: <b>{dataAfterPost.secret_key}</b></p>
            <p>Tanggal Expired: <b>{dataAfterPost.expired_at}</b></p>
            <p>Aktif: <b>{dataAfterPost.is_active === 1 ? 'Active' :  'Inactive'}</b></p>
          </div>
        </div>
      ) : (
        ""
